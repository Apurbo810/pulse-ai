...
- Added CPU process analysis engine with severity classification:
  - Medium
  - High
  - Critical
- Added application-specific CPU optimization recommendations for:
  - Chrome
  - Microsoft Edge
  - Firefox
  - Discord
...
### Changed
- Refactored CPU monitoring to use a single sampling source instead of sampling from renderer requests.
...
- Updated display information retrieval to reuse the same cached graphics snapshot.
- Improved monitoring architecture to reduce unnecessary PowerShell/WMI executions.
- Improved CPU reading stability by preventing sampling interference from concurrent requests.
- Simplified renderer-side monitoring logic.
...
- Improved overall responsiveness of the monitoring subsystem.
